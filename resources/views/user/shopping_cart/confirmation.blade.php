<x-layout>
    <x-slot:heading>Confirmación</x-slot:heading>
    <main class="pt-24">
        <section class="container mx-auto px-4 text-center">
            <h2 class="text-3xl font-bold mb-6">¡Gracias por tu compra!</h2>
            <p class="text-lg mb-4">Tu pedido ha sido procesado y el pago se realizó correctamente.</p>
            <a href="{{ route('checkout.invoice') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg">
                Descargar Factura
            </a>

            <a href="/products" class="bg-blue-500 text-white px-6 py-3 rounded-lg hover:cursor-pointer">Volver al
                inicio</a>
        </section>
    </main>
</x-layout>