            ...
            <div class="flex items-center space-x-4">
                ...
                <a href="{{ route('shopping_cart.index') }}" class="text-gray-500 hover:text-primary-green relative">
                    ...
                    @if(Cart::instance('cart_' . auth()->id())->content()->count() > 0)
                        <span
                            class="absolute -top-2 -right-2 bg-primary-green text-black text-xs rounded-full h-5 w-5 flex items-center justify-center">
                            {{ Cart::instance('cart_' . auth()->id())->content()->count() }}
                        </span>
                    @endif
                </a>

                <!-- Icono Wishlist (corazón) -->
                <a href="/wishlist" class="text-gray-500 hover:text-primary-green relative">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
